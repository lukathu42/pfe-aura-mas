...
\subsection{Task-Allocation Scope and Null Baselines}

The allocation instance in this thesis is intentionally small: one uncertain
hypothesis is offered to a set of cameras, one camera is selected, and the
decision is immediate. It is therefore closer to a single-task,
single-assignment allocation problem than to a continuous fleet-planning
problem. This scope matters when interpreting a market mechanism. At a
two-camera site, a deterministic round-robin or ``other camera'' rule is a
strong and transparent null: a complex allocation protocol must exploit real
information about viewpoint, load, or expected evidence to beat it. If that
information is absent, using an auction changes message flow without proving a
better decision.

For larger sites, assignment can be formulated as a utility matrix over tasks
and cameras and solved by a Hungarian-style centralized matcher, or by
distributed bundle methods. Neither is implemented here. They are useful
future baselines precisely because they distinguish the question ``should a
camera verify?'' from ``which camera is best placed to verify?'' The campaign
audit in Chapter~\ref{chap:evaluation} shows why this distinction cannot be
assumed from the presence of an auction alone.
...
This chapter introduced agents, multi-agent coordination, and agentic AI. It explains why AURA-MAS uses an auction for verification and keeps the explanation layer outside the alert decision. The next chapter reviews the perception methods used at the edge.
